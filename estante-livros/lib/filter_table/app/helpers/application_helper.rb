      ...
      module ApplicationHelper
        ...
        module InstanceMethods
          ...
          def values_from_options(field, options)
            raise ArgumentError, 'missing :values argument' unless options[:values]

            values = options.delete(:values)
            if values == :auto
              raise ArgumentError, 'missing :model argument' unless options[:model]
          ...
          def prepare_values(values)
            ...
            values = values.sort_by { |a| a[1] } if values.is_a? Hash
            # remove nil
            values.delete_if { |name, _value| name.nil? }
          ...
          def already_filtered?(field, value)
            ...
            params.key?(field) && param_values.include?(value)
          end
          ...
          def add_filter_url(field, value)
            new_value = value
            new_value = "#{params[field]} #{value}" if params.key? field
